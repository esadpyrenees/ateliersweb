...
*::before { content: "*"; color:var(--color, white); font-size:20px}
* {
	position: absolute;
...
`);
      ...
      let colors = ["tomato","DeepSkyBlue","blueviolet","DarkSeaGreen","DeepPink","yellowgreen"];
...
:nth-child(${j}) {
  --color: ${color};
	...
	transform: translate(${x}${units.x}, -10px) ;
	animation: fall-${j} ${duration}s ${delay}s linear infinite;
}

@keyframes fall-${j} {
	${yoyo}% {
		transform: translate(${x + offset}${units.x}, ${yStart}${units.y}) ;
	}

	to {
		transform: translate(${x + offset / 2}${units.x}, ${yEnd}${units.y}) ;
	}
}`)
		}
		return css.join("\n");
	...
	connectedCallback() {
		...
		let count = parseInt(this.getAttribute(Snow.attrs.count)) || 30;
